...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.title in dresses) {
      if (!dresses[item.title].colour.includes(item.colour) && item.availableQty > 0) {
        dresses[item.title].colour.push(item.colour)
      }
      if (!dresses[item.title].size.includes(item.size) && item.availableQty > 0) {
